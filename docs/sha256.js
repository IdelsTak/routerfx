...
function sha256_transform() {
    ...
    f = ihash[7];
    for (var c = 0; c < 16; c++)
        u[c] = buffer[3 + (c << 2)] | buffer[2 + (c << 2)] << 8 | buffer[1 + (c << 2)] << 16 | buffer[c << 2] << 24;
    for (var _ = 0; _ < 64; _++)
        s = f + sha256_Sigma1(e) + choice(e, i, n) + K256[_],
        s += _ < 16 ? u[_] : sha256_expand(u, _),
        o = sha256_Sigma0(a) + majority(a, h, r),
        f = n,
        n = i,
        i = e,
        e = safe_add(t, s),
...
function sha256_update(a, h) {
    ...
    t = count[0] >> 3 & 63;
    var i = 63 & h;
    for ((count[0] += h << 3) < h << 3 && count[1]++,
    count[1] += h >> 29,
    r = 0; r + 63 < h; r += 64) {
        for (var n = t; n < 64; n++)
            buffer[n] = a.charCodeAt(e++);
        sha256_transform(),
        t = 0
    }
    for (n = 0; n < i; n++)
...
function sha256_encode_bytes() {
    for (var a = 0, h = new Array(32), r = 0; r < 8; r++)
        h[a++] = ihash[r] >>> 24 & 255,
        h[a++] = ihash[r] >>> 16 & 255,
        h[a++] = ihash[r] >>> 8 & 255,
        h[a++] = 255 & ihash[r];
...
function sha256_encode_hex() {
    for (var a = new String, h = 0; h < 8; h++)
        for (var r = 28; r >= 0; r -= 4)
            a += sha256_hex_digits.charAt(ihash[h] >>> r & 15);
    return a
}
