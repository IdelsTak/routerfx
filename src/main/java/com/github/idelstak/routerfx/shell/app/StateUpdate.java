...
public final class StateUpdate implements Update {
    ...
    private AppState connect(AppState state, String baseUrl, Credentials credentials) {
        return new AppState(
          ...
          new DashboardState(state.dashboard().common(), Optional.empty(), Optional.empty(), false, state.dashboard().updates()),
    ...
    private AppState refresh(AppState state) {
        return new AppState(
          state.login(),
          new DashboardState(state.dashboard().common(), state.dashboard().radio(), Optional.empty(), true, state.dashboard().updates()),
    ...
    private AppState authenticate(AppState state, Session session, RadioState radio) {
        return new AppState(
          ...
          new DashboardState(state.dashboard().common(), Optional.of(radio), Optional.empty(), false, state.dashboard().updates() + 1),
    ...
    private AppState common(AppState state, CommonDashboard common) {
        return new AppState(
          state.login(),
          new DashboardState(Optional.of(common), state.dashboard().radio(), Optional.empty(), false, state.dashboard().updates() + 1),
